...
def load_model_finetune(model_name_or_path, adapter_path):
...
def main(): 
    ...
    llm = load_model_finetune(model_name_or_path, adapter_path)
    ...
    filename = f"{adapter_path}-Evaluate-{date_time}.csv"
    df.to_csv(filename)
    print("finish")
